...
img_hist_4dfp -b$bins "-r"$lowlimit"to"$hilimit $img -m$mask -x | tail -$bins | head -$xlines >! temp_hist.txt
...
set mean = `qnt_4dfp $img $mask | tail -2 | head -1 | awk '{print $2}'`
set mean70 = `qnt_4dfp -v$binlow"to"$binhi $img $mask | tail -2 | head -1 | awk '{print $2}'`
# echo "mean = "$mean
# echo "70% mean = "$mean70
